...
void funcion(float A, float B, float t1, float t2, float i){
    glBegin(GL_POLYGON);
            
            float startAngle = t1 + i*(t2-t1)/segmentos;
            float endAngle = t1 + (i+1)*(t2-t1)/segmentos;
            
            glVertex2f(A*cos(startAngle) , B*sin(startAngle));
            glVertex2f(A*cos(endAngle) , B*sin(endAngle));
            
            glVertex2f(A*cos(t1) , B*sin(t1));
            glVertex2f(A*cos(t2) , B*sin(t2));
    glEnd();
}
...
void funcion(
    ...
    glBegin(GL_POLYGON);
            ...
            glVertex2f(B*cos(startAngle) , a*(B*cos(startAngle)-A*cos(t3)) + cy);
            glVertex2f(B*cos(endAngle) , a*(B*cos(endAngle)-A*cos(t3)) + cy);
            
            glVertex2f(B*cos(endAngle) , 0);
            glVertex2f(B*cos(startAngle) , 0);
...
void batman(){
    ...
    for(int i=0; i<segmentos;i++){
        funcion(A,B,-PI/4,PI/3,i);
        funcion(A,B, 2*PI/3,5*PI/4,i);
        
        funcion(A, B, 4*M_PI/9 , 13.0*M_PI/27, 4.0*M_PI/9, 2.3, 0.527, i);
        funcion(A, B, 14.0*M_PI/27 , 5*M_PI/9, 4.0*M_PI/9, -2.3, -0.001, i);
