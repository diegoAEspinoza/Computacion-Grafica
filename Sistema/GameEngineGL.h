...
class GameEngine
{
	uint32_t m_ColourPalette[17] = // 0xAABBGGRR
	{
		0xFF000000, // BLACK
	...
	{
		static GameEngine *cge;


		switch (uMsg)
		{
		case WM_CREATE:
			cge = static_cast<GameEngine*>(((LPCREATESTRUCT)lParam)->lpCreateParams);
	...
	{
		wchar_t wnd_title[] = L"Game Engine (OGL)";
		wchar_t wnd_class[] = L"GAME_ENGINE_CLASS";
	...
	void GameThread()
	{
		...
		while (m_bAtomActive)
		{
			...
			while (m_bAtomActive)
			{
				...
				swprintf_s(sNewTitle, 256, L"Game Engine (OGL) - %s - FPS: %3.2f", m_sAppName.c_str(), 1.0f / fElapsedTime);
...
atomic<bool> GameEngine::m_bAtomActive(false);
condition_variable GameEngine::m_cvGameFinished;
mutex GameEngine::m_muxGame;